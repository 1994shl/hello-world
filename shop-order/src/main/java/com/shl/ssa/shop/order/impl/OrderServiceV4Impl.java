...
public class OrderServiceV4Impl implements OrderService {
    ...
    public void saveOrder(OrderParams orderParams) {
        ...
        UserEntity user = restTemplate.getForObject(
                "http://" + userServer + "/user/get/" + orderParams.getUserId(), UserEntity.class);
        if (ObjectUtils.isEmpty(user)) {
            throw new RuntimeException("未获取到用户信息: " + JSONObject.toJSONString(orderParams));
        }
        ProductEntity product = restTemplate.getForObject(
                "http://" + productServer + "/product/get/" + orderParams.getProductId(), ProductEntity.class);
        if (product == null) {
        ...
        Result<Integer> result = restTemplate.getForObject(
                "http://" + productServer + "/product/update_count/" + orderParams.getProductId() + "/" + orderParams.getCount(), Result.class);
